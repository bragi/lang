...
{
    LangParser *parser = [[LangParser alloc] initWithCode:codeText andRuntime:runtime];
    LMessage *result = [parser parse];
    NSLog(@"Parsed code is:\n%@", [result stringValue]);
    LangOperators *operators = [[LangOperators alloc] init];
    result = [operators shuffle:result];
    NSLog(@"Shuffling done");
    NSLog(@"Parsed code is:\n%@", [result stringValue]);
    return result;
}
...
- (LMessage*)parseText:(unichar)start
{
    ...
    switch (start) {
        case '"':
            while ((rr = [self peek]) != '"' && rr != 0) {
                // TODO: add escaping
                [self read];
                [name appendFormat:@"%C", rr];
            }
            // Read closing quotation
            [self read];
            break;
        default:
            break;
